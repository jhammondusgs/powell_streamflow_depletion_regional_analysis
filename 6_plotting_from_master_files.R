# first section of code is for plotting data for regional gages (California, Michigan, Kansas)
# section starting line 233 is for gages across CONUS. CONUS code section has more features.
...
###################################
###################################
# Same as above, but for national locations
###################################
###################################
library(googledrive)
library(ggplot2)
...
base_wd <- "/Users/johnhammond/powell_streamflow_depletion_regional_analysis/"
setwd(base_wd)
dir.create(paste0(base_wd, "master_files_national"))
setwd(paste0(base_wd, "master_files_national"))
# access files from google drive
my_url <- "https://drive.google.com/drive/folders/1GS31PaawF0AGGuGnwQ3ThpFSkOd85-XZ"
x <- drive_ls(as_id(my_url))
y <- drive_ls(x$id[2]) # master files folder on google drive
...
base_breaks <- function(n = 10){function(x) {axisTicks(log10(range(x, na.rm = TRUE)), log = TRUE, n = n)}}
### use maser files to plot data
seasonal <- read.csv("seasonal_streamflow_metrics_NATIONAL_04212023.csv")
annual <- read.csv("water_year_streamflow_metrics_and_climate_data_NATIONAL_04212023.csv")
trends1921 <- read.csv("trends_in_19212020_water_year_streamflow_metrics_and_climate_data_NATIONAL_04212023.csv")
trends1951 <- read.csv("trends_in_19512020_water_year_streamflow_metrics_and_climate_data_NATIONAL_04212023.csv")
trends1981 <- read.csv("trends_in_19812020_water_year_streamflow_metrics_and_climate_data_NATIONAL_04212023.csv")
trends1921_seasonal <- read.csv("trends_in_19212020_seasonal_streamflow_metrics_NATIONAL_04212023.csv")
trends1951_seasonal <- read.csv("trends_in_19512020_seasonal_streamflow_metrics_NATIONAL_04212023.csv")
trends1981_seasonal <- read.csv("trends_in_19812020_seasonal_streamflow_metrics_NATIONAL_04212023.csv")
...
ggplot(data = states)+ geom_polygon(data = states,aes(x = long, y = lat, group = group), fill = NA, color = "black") +
  theme_void() + 
  geom_point(data = longterm_annual_median, aes(x=dec_long_va, y=dec_lat_va, col = daysbelowlowflowthreshannual)) +
  # lines commented out below could dbe used to plot sites with different symbols for pumping affected sites vs natural sites
  #geom_point(data = longterm_annual_median, aes(x=dec_long_va, y=dec_lat_va, col = median, shape = factor(HCDN.2009), size = factor(HCDN.2009), alpha = factor(HCDN.2009))) +
  #scale_shape_manual(values=c(19, 17))+
  #scale_size_manual(values = c(0.75,2))+
...
ggplot(data = states)+ geom_polygon(data = states,aes(x = long, y = lat, group = group), fill = NA, color = "black") +
  theme_void() + 
  geom_point(data = subset(longterm_seasonal_median, longterm_seasonal_median$season == "summer"),
             aes(x=dec_long_va, y=dec_lat_va, col = daysbelowlowflowthresh)) +
  scale_color_viridis(option = "turbo",  direction = -1)+
  coord_map("albers", lat0=30, lat1=40)
...
length(unique(trends1981_wide$gage))
...
ggplot(data = trends1951_wide, aes(x = tau_wy_p_mm, y = tau_daysbelowlowflowthreshannual))+geom_point()
ggplot(data = trends1951_wide, aes(x = tau_octjun_spei_6m, y = tau_volumebelowlowflowthreshannual))+geom_point()
